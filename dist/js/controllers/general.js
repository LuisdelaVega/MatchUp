...
myApp.controller('generalViewController', ['$scope', '$http', '$state', 'sharedDataService', '$window', '$rootScope', '$q',
function ($scope, $http, $state, sharedDataService, $window, $rootScope, $q) {
		...
		$scope.filter = function (filterBy) {
			...
			if (filterBy == 'All')
				...
				filterBy = '&magnitude=' + filterBy;
			$q.all(
			[
				$http.get($rootScope.baseURL + '/matchup/events?type=hosted&state=upcoming' + filterBy),
				$http.get($rootScope.baseURL + '/matchup/events?type=hosted&state=live' + filterBy),
				$http.get($rootScope.baseURL + '/matchup/events?type=regular&state=upcoming' + filterBy),
				$http.get($rootScope.baseURL + '/matchup/events?type=regular&state=live' + filterBy),
			])
				.then(function (results) {
				console.log(results[0].data);
					$scope.hostedEvents = results[0].data;
					$scope.hostedLiveEvents = results[1].data
					$scope.regularEvents = results[2].data;
					$scope.regularLiveEvents = results[3].data;
