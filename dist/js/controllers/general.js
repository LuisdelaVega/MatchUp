...
function($scope, $http, $state, sharedDataService, $window, $rootScope, $q) {
	...
	$scope.filter = function(filterBy, state) {
		...
		$q.all([$http.get($rootScope.baseURL + '/matchup/events?type=hosted' + state + filterBy), $http.get($rootScope.baseURL + '/matchup/events?type=regular' + state + filterBy)]).then(function(results) {
	...
	$scope.filterState = function(filterBy) {
		$scope.filterBy = filterBy;
		if (filterBy == 'All')
			filterBy = '';
		else
			filterBy = '&magnitude=' + filterBy;
		$q.all([$http.get($rootScope.baseURL + '/matchup/events?type=hosted' + filterBy), $http.get($rootScope.baseURL + '/matchup/events?type=regular&state=past' + filterBy)]).then(function(results) {
