...
myApp.controller("tournamentDetailsController", function($scope, $http, $window, $rootScope, $state, $stateParams) {
	...
	$scope.init = function() {
		...
		//Winners Round
		$scope.winnerRoundsExist = false;
		$scope.winnerRoundsCompleted = true;

		//Round Robin
		$scope.roundRobinRoundsExist = false;
		$scope.roundRobinRoundsCompleted = true;
		...
		$http.get($rootScope.baseURL + '/matchup/events/' + $stateParams.eventName + '/tournaments/' + $stateParams.tournamentName + '/rounds?date=' + $stateParams.eventDate + '&location=' + $stateParams.eventLocation).success(function(data, status) {
			...
			console.log(data);
			// if($scope.tournament.tournament_format == "Round Robin"){
			// //var temp = $scope.tournamentInfo.finalStage.roundRobinRounds;
			// $scope.tournamentInfo.finalStage.winnerRounds = $scope.tournamentInfo.finalStage.roundRobinRounds;
			// }
	...
	$scope.finalStagesCheck = function() {
		...
		//Just  checking if round robin  round exist
		if ($scope.tournamentInfo.finalStage.roundRobinRounds) {
			$scope.roundRobinRoundsExist = true;

			//iterating over all the rounds in the winners rounds to see if all of these have been completed, if not then winners bracket has not been finished
			for (var j = 0; $scope.tournamentInfo.finalStage.roundRobinRounds.length > j; j++) {
				if (!$scope.tournamentInfo.finalStage.roundRobinRounds[j].round_completed) {
					$scope.roundRobinRoundsCompleted = false;
				}
	...
	$scope.assignDropdowns = function() {
		...
		} else if ($scope.roundRobinRoundsExist) {
			$scope.stages = ['Round Robin Round'];
		} else {
			//what???? how!?!?!?
		}

		if ($scope.initial) {
			if ($scope.roundRobinRoundsExist) {
				$scope.roundOf = 'Round Robin';
				$scope.selectedStage = 'Round Robin Round';
				$scope.rounds = $scope.tournamentInfo.finalStage.roundRobinRounds;
				$scope.selectedRound = $scope.tournamentInfo.finalStage.roundRobinRounds[0];
			} else if ($scope.winnerRoundsExist) {
	...
	$scope.changeSelectedStage = function() {
		...
		} else if ($scope.selectedStage == 'Round Robin Round') {
			$scope.rounds = $scope.tournamentInfo.finalStage.roundRobinRounds;
			$scope.selectedRound = $scope.rounds[0];
			$scope.index = 0;
			$scope.roundOf = 'Winner';
	...
	$scope.changeBestOf = function() {
		...
		if ($scope.checkScore()) {
			...
			if (!$scope.selectedRound.round_best_of) {
				alert("Please choose a number for the amount of sets (Best of)");
				return;
			}

			if (isNaN($scope.selectedRound.round_best_of)) {
				alert("The entered value is not a number");
				return;

			}
			if (!$scope.selectedRound.round_best_of % 2) {
			...
			$http.post($rootScope.baseURL + '/matchup/events/' + $stateParams.eventName + '/tournaments/' + $stateParams.tournamentName + '/rounds/' + $scope.selectedRound.round_number + '?date=' + $stateParams.eventDate + '&location=' + $stateParams.eventLocation + '&round_of=' + $scope.roundOf, {
				"best_of" : $scope.selectedRound.round_best_of
			}).success(function(data) {
				//$scope.tournaments = data;
				alert("Succesfully changed the amount of sets to be best of " + $scope.selectedRound.round_best_of);
				$('#changeBestOfPrompt').modal('hide');
	...
	$scope.changeDate = function() {
		$scope.init();
		var date = new Date($scope.selectedRound.round_start_date);

		if (date < new Date($scope.eventInfo.event_start_date)) {
			alert("Round cannot start before the event starts!");
			return;
		}
		if (date > new Date($scope.eventInfo.event_end_date)) {
			alert("Round cannot start after the event ends!");
			return;
		}
		if (date < $scope.tournament.tournament_start_date) {
			alert("Round cannot start before the tournament starts!");
			return;
		}
		console.log("Change Date");
		$http.put($rootScope.baseURL + '/matchup/events/' + $stateParams.eventName + '/tournaments/' + $stateParams.tournamentName + '/rounds/' + $scope.selectedRound.round_number + '?date=' + $stateParams.eventDate + '&location=' + $stateParams.eventLocation + '&round_of=' + $scope.roundOf, {
			"start_date" : $scope.selectedRound.round_start_date
		}).success(function(data) {
			alert("Succesfully changed the Round Start Date!");
		}).error(function(err) {
			console.log(err);
		});
	};
