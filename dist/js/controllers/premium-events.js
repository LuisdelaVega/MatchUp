...
function ($scope, $http, $state, $stateParams, $window, $rootScope) {
		...
		//matchup/events/:event/meetups/:username
		$scope.deleteMeetup = function (index, meetup_user, meetup_date, meetup_location) {
			//Get Event Info from server
			$http.delete($rootScope.baseURL + '/matchup/events/' + $stateParams.eventname + '/meetups/' + meetup_user + '?date=' + $stateParams.date + '&location=' + $stateParams.location + '&meetup_date=' + meetup_date + '&meetup_location=' + meetup_location).success(function (data, status, headers) {
				$scope.meetupsInfo.splice(index, 1);
			});
...
function ($scope, $http, $state, $stateParams, $window, $rootScope) {
		...
		$scope.deleteMeetup = function (meetup_user, meetup_date, meetup_location) {
			//Get Event Info from server
			$http.delete($rootScope.baseURL + '/matchup/events/' + $scope.eventInfo.event_name + '/meetups/' + meetup_user + '?date=' + $scope.eventInfo.event_start_date + '&location=' + $scope.eventInfo.event_location + '&meetup_date=' + meetup_date + '&meetup_location=' + meetup_location).success(function (data, status, headers) {
				alert("Succesfully deleted the meetup!");
				$state.go("app.meetupList", {
					"eventname": $scope.eventInfo.event_name,
					"date": $scope.eventInfo.event_start_date,
					"location": $scope.eventInfo.event_location
				});
			});
