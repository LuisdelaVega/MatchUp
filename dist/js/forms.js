...
myApp.controller("CreateEventController", function ($scope) {
	...
	// Get usertag of the current user
	$scope.userTag = [{
		name: 'Tag',
				}];

	//Initiate get to retrieve the organization that the
	...
	// Push usertag into the organization array
	// This will be used to initializethe Hosted by select dropdown form
	$scope.userOrganization.unshift($scope.userTag[0]);

	// Init event type object for the dropdown
	$scope.eventType = [
		{
			name: 'Local',
				}, {
	...
	$scope.validateTournament = function () {
		...
		var tournament = {
			...
			"deduction_fee": $scope.tournament.deduction_fee,
