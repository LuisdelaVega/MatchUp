	...
	pg.connect(conString, function(err, client, done) {
		...
		query.on("end", function(result) {
			if (result.rows.length) {
				...
				var query = client.query({
					text : "SELECT distinct competitor.competitor_number, competitor.competitor_seed FROM tournament LEFT OUTER JOIN is_a ON is_a.event_name = tournament.event_name AND is_a.event_start_date = tournament.event_start_date AND is_a.event_location = tournament.event_location AND is_a.tournament_name = tournament.tournament_name JOIN competitor ON is_a.event_name = competitor.event_name AND is_a.event_start_date = competitor.event_start_date AND is_a.event_location = competitor.event_location AND is_a.tournament_name = competitor.tournament_name AND is_a.competitor_number = competitor.competitor_number LEFT OUTER JOIN competes_for ON competes_for.event_name = tournament.event_name AND competes_for.event_start_date = tournament.event_start_date AND competes_for.event_location = tournament.event_location AND competes_for.tournament_name = tournament.tournament_name JOIN competitor ON tournament.event_name = competitor.event_name AND tournament.event_start_date = competitor.event_start_date AND tournament.event_location = competitor.event_location AND tournament.tournament_name = competitor.tournament_name AND (is_a.competitor_number = competitor.competitor_number OR competes_for.competitor_number = competitor.competitor_number) WHERE tournament.event_name = $1 AND tournament.event_start_date = $2 AND tournament.event_location = $3 AND tournament.tournament_name = $4 AND competitor_check_in ORDER BY competitor.competitor_seed",
					values : [req.params.event, req.query.date, req.query.location, req.params.tournament]
				});
				query.on("row", function(row, result) {
					result.addRow(row);
...
function getGroupHistory(req, res, client, done, log, standings, players, index, length, getStandingsForFinalStage) {
	for (var i = 0; i < players.length; i++) {
					...
					query.on("end", function (result) {
						...
						query.on("end", function (result) {
							for (var i = 0; i < standings.groupStage.groups.length; i++) {
								getGroupHistory(req, res, client, done, log, standings, standings.groupStage.groups[i], i, standings.groupStage.groups.length-1, getStandingsForFinalStage);
							}
						});
					});
				} else {
