...
var getStandings = function(req, res, pg, conString, log) {
	pg.connect(conString, function(err, client, done) {
		...
		var query = client.query({
			text: "SELECT tournament_type, team_size FROM tournament NATURAL JOIN event WHERE event_name = $1 AND event_start_date = $2 AND event_location = $3 AND tournament_name = $4 AND event_active AND tournament_active",
			values: [req.params.event, req.query.date, req.query.location, req.params.tournament]
		});
		query.on("row", function (row, result) {
			result.addRow(row);
				...
				if (result.rows[0].tournament_type == "Two Stage") {
					...
					query.on("end", function (result) {
						...
						if (!result.rows[0].stage_completed) {
							...
							var queryText = '';
							if (parseInt(details.team_size) > 1) {
							...
							var query = client.query({
								text: queryText,
								values: [req.params.event, req.query.date, req.query.location, req.params.tournament, "Group", ",;!;,", "Winner", "Round Robin", "Loser"]
							});
							query.on("row", function (row, result) {
								var repeated = false;
								var info_string = row.info;
			...
			if (result.rows.length) {
				...
				query.on("end", function(result) {
					...
					if (result.rows.length) {
						...
						console.log("The match already had a station with number: ");
						...
						var query = client.query({
							text: "SELECT match_number FROM is_played_in NATURAL JOIN match NATURAL JOIN round NATURAL JOIN tournament WHERE event_name = $1 AND event_start_date = $2 AND event_location = $3 AND tournament_name = $4 AND round_number = $5 AND round_of = $6 AND station_number = $7",
						...
						query.on("end", function(result) {
							if (result.rows.length) {
								console.log("A match played in this station was found. Match number: ");
								...
								console.log("No matches are being played in the provided station");
