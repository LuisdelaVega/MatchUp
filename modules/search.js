			...
			queryLive.on("end", function(result) {
				...
				queryPast.on("end", function(result) {
					...
					var queryRegular = client.query({
						text : "SELECT event_name, event_start_date, event_end_date, event_location, event_venue, event_logo FROM event WHERE event_name ILIKE '%" + req.params.parameter + "%' AND event_start_date > now() at time zone 'utc' AND event_name NOT IN (SELECT event.event_name FROM event NATURAL JOIN hosts) AND event_active ORDER BY event.event_start_date"
					});
					...
					queryRegular.on("end", function(result) {
						...
						var queryHosted = client.query({
							text : "SELECT event_name, event_start_date, event_end_date, event_location, event_venue, event_logo FROM event NATURAL JOIN hosts WHERE event.event_name ILIKE '%" + req.params.parameter + "%' AND event.event_start_date > now() at time zone 'utc' AND event_active ORDER BY event.event_start_date"
						});
