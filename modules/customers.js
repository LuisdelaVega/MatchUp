	...
	query.on("end", function(result) {
		if (result.rows.length) {
			client.query({
				...
				values : [req.body.event.name, req.body.event.start_date, req.body.event.location, tournament.name, tournament.game, tournament.rules, tournament.teams, tournament.start_date, tournament.deadline, Math.floor(tournament.fee * 100) / 100, tournament.capacity, Math.floor(tournament.seed_money * 100) / 100, tournament.type, tournament.format, tournament.scoring, ((tournament.type === "Two Stage") ? tournament.group_players : 0), ((tournament.type === "Two Stage") ? tournament.group_winners : 0), tournament.prize_distribution]
			}, function(err, result) {
				if (err) {
					client.query("ROLLBACK");
					done();
